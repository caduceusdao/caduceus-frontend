...
import { defaultDataChainID } from "../constants";
...
function getFujinetURL() {
  return "https://api.avax-test.network/ext/bc/C/rpc";
}
function getBscTestnetURL() {
...
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
  ...
  const [chainID, setChainID] = useState(defaultDataChainID);
  const [address, setAddress] = useState("");
  let uri_;
  switch (defaultDataChainID) {
    case 250:
      uri_ = getMainnetURI();
      break;
    case 4002:
      ...
      break;
    case 31337:
      uri_ = getLocalhostURI();
      break;
      case 43113:
        uri_= getFujinetURL();
        break;
        case 97:
        uri_= getBscTestnetURL();
        break;
  }

  

  const [uri, setUri] = useState(uri_);
  ...
  //     if (otherChainID === 250 || otherChainID === 4002 || otherChainID === 31337) {
  ...
  const _checkNetwork = (otherChainID: number): boolean => {
    if (chainID !== otherChainID) {
      console.warn("You are switching networks");
      if (otherChainID === 1 || otherChainID === 4 || otherChainID === 31337 || otherChainID === 4002 || otherChainID === 43113) {
        setChainID(otherChainID);
        switch (otherChainID) {
          case 1:
            setUri(getMainnetURI());
          ...
          case 31337:
            ...
            break;
            case 43113:
              setUri(getFujinetURL());
              break;
              case 97:
              setUri(getBscTestnetURL());
              break;
        } 
        return true;
      }

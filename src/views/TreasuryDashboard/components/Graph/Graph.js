...
import {
  CaduceusLineChart,
  CaduceusStackedLineChart,
  CaduceusMultiLineChart,
  CaduceusPieChart,
  CaduceusComposedineChart,
  trimNumber,
} from "src/components/Chart/CaduceusChart.jsx";
...
export const TotalValueDepositedGraph = () => {
  ...
  // console.log("TotalValueDepositedGraph: ", data);
  return (
    <CaduceusLineChart
      data={data}
...
export const OhmMintedPerTotalSupplyGraph = () => {
  ...
  const minted =
    ...
    data
      ...
      .slice(0, data.length - 1);
  const fiveDaySlice = minted && minted.slice(0, 5);
  console.log(fiveDaySlice);
  let fiveDayTotal = 10;
  // if (fiveDaySlice !== undefined) {
  //   for (let i = 0; i < 5; i++) {
...
export const GrowthOfSupply = () => {
  ...
  return (
    <CaduceusMultiLineChart
      ...
      headerSubText={`${formattedData && trim(Math.abs(1 - formattedData[0].indexCircSupply / formattedData[0].circSupply) * 100, 2)
        }%`}
      headerSubText="-0.5%"
      dataFormat={["number"]}
